...
> Learn about contrastive learners and one take on how to attack them from a recent paper 
...
A **poisoning** attack is when an adversary introduces malicious training examples to a machine learning classifier so that it outputs an adversarially-desired label for a certain input. For example, we introduce images of a certain black cat with the label "dog" so that in practice black cats are misclassified (so-called "label flipping"). Or, we add human-imperceptible noise to images of black cats such that the model is unable to succesfully learn that the correct label is cat. 
